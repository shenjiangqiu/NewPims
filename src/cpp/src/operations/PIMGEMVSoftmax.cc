...
std::vector<Ptr<BTensor>> PIMGEMVSoftmax::get_outputs(
...
void PIMGEMVSoftmax::initialize_tiles() {
    ...
    for (int i = 0; i < _batch_size; i++) {
        ...
        for (int chunk = 0; chunk < _chunks; chunk++) {
            ...
            uint64_t p_header_addr =
                AddressConfig::encode_pim_header(ch, query_row, true, 0, 0);
            ...
            for (int ti = 0; ti < tiles_per_chunk; ti++) {
                ...
                //     spdlog::info("{}num_comps: {}, actual:{},  {}", color,
                //     num_comps,
                ...
                //     else
                //         color = red;
                //     spdlog::info("{}num_readres: {}, actual:{},  {}", color,
                //     num_readres,
...
void PIMGEMVSoftmax::calculate_loops() {
    ...
    uint32_t page_size = _config.dram_page_size /
                         _config.precision;  // number of parameters in dram row
    ...
    std::string yellow = "\033[1;33m";
    std::string reset = "\033[0m";
    spdlog::info("{}chunks:{}, heads_per_tile:{}, comps_per_head:{} {}", yellow,
                 _chunks, _heads_per_tile, _comps_per_head, reset);
}
...
uint32_t PIMGEMVSoftmax::sram_size_needed() {
    ...
    uint32_t need_size =
        total_seq_len * _config.model_n_head * _config.precision;
