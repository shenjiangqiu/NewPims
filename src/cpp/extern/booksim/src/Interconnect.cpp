...
Interconnect::Interconnect(const std::string ConfigFilePath,
  ...
  initParameters();
  int _num_subnets = Config.GetInt("subnets");
  if (_num_subnets <=0){
    throw std::runtime_error("subnets must be greater than 0");
  }
  this->num_subnets = _num_subnets;
  ...
  for (unsigned s = 0; s < num_subnets; ++s) {
