...
Command NewtonCommandQueue::GetCommandToIssue(
...
bool NewtonCommandQueue::WillAcceptCommand(int rank, int bankgroup,
                                           int bank) const {
    if (rank == -1) {
    ...
    if (pim_queue_.size() > 0) {
        ...
        //     PrintDebug("(WillAcceptCommand) cid:", channel_id_, "pim_queue is
        //     not empty");
        return false;
    }
...
CMDQueue &NewtonCommandQueue::GetQueue(bool is_pimq_cmd, int rank,
...
bool NewtonCommandQueue::CanMeetRefreshDeadline(
...
Command NewtonCommandQueue::GetReadyInPIMQueue(
    std::pair<int, int> refresh_slack) {
    ...
    for (auto cmd_it = queue.begin(); cmd_it != queue.end(); cmd_it++) {
        if (is_gwriting_) {
            ...
            if (can_issue_gwrite) {
                ...
                PrintWarning(ColorString(Color::BLUE), "cid:", channel_id_,
                             "skip_pim ON", "gwrite//");
                return Command();
            }
        }
        ...
        if (cmd_it->IsPIMHeader()) {
            ...
            if (can_issue_gemv) {
                PrintInfo(channel_id_,
                          "pim_mode: ON!!!! >> target row:", cmd_it->Row());
                is_pim_mode_ = true;
...
Command NewtonCommandQueue::GetFirstReadyInQueue(
    ...
    for (auto cmd_it = queue.begin(); cmd_it != queue.end(); cmd_it++) {
        if (reserved_row_for_pim_ == cmd_it->Row()) {
            PrintError("cid:", channel_id_,
                       "reserved_row_for_pim_:", reserved_row_for_pim_,
...
void NewtonCommandQueue::ErasePIMCommand(const Command &cmd) {
    // if (!is_pim_mode_)
    //     PrintDebug(ColorString(Color::RED), "try to (ErasePIMCommand) cid:",
    //     channel_id_,
    //                "cmd:", cmd.CommandTypeString(), "addr:", cmd.hex_addr);
    assert(is_pim_mode_ || is_gwriting_);
    assert(cmd.IsPIMHeader() || cmd.IsPIMComp() || cmd.IsReadRes() ||
           cmd.IsGwrite());
...
bool NewtonCommandQueue::HasRWDependency(const CMDIterator &cmd_it,
                                         const CMDQueue &queue) const {
