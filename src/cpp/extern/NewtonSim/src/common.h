...
struct Command {
    ...
    // for COMPS_READRES
    Command(CommandType cmd_type, const Address &addr, uint64_t hex_addr,
            bool is_last, int num_comps)
        : cmd_type(cmd_type),
          addr(addr),
          hex_addr(hex_addr),
          for_gwrite(false),
          is_last_comps(is_last),
          num_comps(num_comps) {}
    // for P_HEADER : for_gwrite is deprecated
    Command(CommandType cmd_type, const Address &addr, uint64_t hex_addr,
            bool for_gwrite, int num_comps, int num_readres)
        : cmd_type(cmd_type),
          addr(addr),
          hex_addr(hex_addr),
          for_gwrite(for_gwrite),
          is_last_comps(false),
          num_comps(num_comps),
    ...
    bool IsChannelCMD() const {
        return cmd_type == CommandType::COMP ||
    ...
    bool IsPIMCommand() const {
        return cmd_type == CommandType::GWRITE ||
    ...
    bool IsCompsReadres() const {
        return cmd_type == CommandType::COMPS_READRES;
    }
    bool IsGwrite() const { return cmd_type == CommandType::GWRITE; }
    bool NeedRefreshCheck() const { return cmd_type == CommandType::P_HEADER; }
    bool IsPIMPrecharge() const {
        return cmd_type == CommandType::PIM_PRECHARGE;
    }
    bool IsNormalBufferCommand() const {
        bool p_header_for_gwrite =
            for_gwrite && cmd_type == CommandType::P_HEADER;

        return cmd_type == CommandType::READ ||
               cmd_type == CommandType::READ_PRECHARGE ||
               cmd_type == CommandType::WRITE ||
               cmd_type == CommandType::WRITE_PRECHARGE ||
               cmd_type == CommandType::ACTIVATE ||
               cmd_type == CommandType::PRECHARGE ||
               cmd_type == CommandType::REFRESH_BANK ||
               cmd_type == CommandType::REFRESH ||
               cmd_type == CommandType::SREF_ENTER ||
               cmd_type == CommandType::SREF_EXIT ||
               cmd_type == CommandType::GWRITE || p_header_for_gwrite;
    }
    bool IsPIMBufferCommand() const {
        bool p_header_for_gemv =
            !for_gwrite && cmd_type == CommandType::P_HEADER;

        return cmd_type == CommandType::G_ACT ||
               cmd_type == CommandType::COMP ||
               cmd_type == CommandType::READRES ||
               cmd_type == CommandType::PIM_PRECHARGE ||
               cmd_type == CommandType::PWRITE || p_header_for_gemv ||
               cmd_type == CommandType::COMPS_READRES;
    }
    bool IsLastPIMCmd() const {
        // `is_last_comps` in COMP does not mean last command
        return is_last_comps && (cmd_type == CommandType::READRES ||
                                 cmd_type == CommandType::COMPS_READRES);
    }
    // <<< gsheo
